...
static function enlazar_adjuntos_original($courseid, $carpeta,$id_email, $juntar, $tiempo )
	 {
		
		$out = array();
		$fs = get_file_storage();
		$context = get_context_instance(CONTEXT_COURSE, $courseid);
		$files = $fs->get_area_files($context->id, 'block_quickmail', 'attachment_'.$carpeta, $id_email); 
		foreach ($files as $file) {
		 $url = "{$CFG->wwwroot}/pluginfile.php/".$file->get_contextid()."/block_quickmail/attachment_".$carpeta;
		...
		//$br = html_writer::empty_tag($juntar); 
		return implode($juntar, $out);
	 }
