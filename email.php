...
if (!empty($type) and !in_array($type, array('log', 'drafts','received','folders_msg'))){
...
if ($form->is_cancelled()) {
    ...
    if (empty($warnings)) {
        ...
        file_save_draft_area_files($data->attachments, $context->id,
            'block_quickmail', 'attachment_' . $table, $data->id, $editor_options);
   ...
   if (!empty($sigs) and $data->sigid > -1) {
		...
		global $SITE;
			...
			$s->url = $CFG->wwwroot.'/message/index.php?user='.$userid.'&id='.$user->id;
		
			$emailtagline = get_string_manager()->get_string('emailtagline', 'message', $s, $userid->lang);
	
			$mesnaje_html=$data->message. "<br /><br />---------------------------------------------------------------------<br />".$emailtagline;
             $success = email_to_user($everyone[$userid], $user, $subject, strip_tags($data->message), $mesnaje_html);
				 
				 $savemessage = new stdClass();
				 $savemessage->userto=$userid;
				$savemessage->userfrom= $user;
				$savemessage->subject=$subject;
				$savemessage->fullmessage=strip_tags($data->message);
				$savemessage->fullmessageformat=FORMAT_HTML;
				$savemessage->fullmessagehtml= $data->message;
				$savemessage->smallmessage=$data->message;
				
					quickmail::reflejar_en_mensajes($savemessage);	
...
$cabecera= $OUTPUT->heading('Quick-Argo');
